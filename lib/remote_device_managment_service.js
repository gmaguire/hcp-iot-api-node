...
var RemoteDeviceManagementService = module.exports = function RemoteDeviceManagementService(options) {
  ...
  this._landscape = options.landscape || "hanatrial.ondemand.com";
  this._hostSuffix = this._landscape.indexOf("hanatrial") >= 0 ? "trial" : ""
  this._host = "iotrdmsiotservices-" + this._account  + this._hostSuffix;
  
  this._path =  "/com.sap.iotservices.dms/api/";
  this._url = this._protocol + this._host + "." + this._landscape + this._path;
  
  this._authInfo = {
    'user': options.user || options.account,
